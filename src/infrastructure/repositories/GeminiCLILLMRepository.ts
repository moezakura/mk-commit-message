...
export class GeminiCLILLMRepository implements ILLMRepository {
...
${diff.getValue()}`;
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('error', (error) => {
        reject(new Error(`Gemini CLI failed to start: ${error.message}`));
      });
      ...
      child.on('close', (code) => {
        ...
        try {
          ...
          reject(new Error(`Failed to parse Gemini output: ${error instanceof Error ? error.message : String(error)}`));
        }
      });
    });
  }
